...
function EnvLinear:get_amp(tick_len)
    local ret
    if self.on then
        ret = math.min(1,self.att_pos/self.att)
        self.att_pos = self.att_pos + tick_len
    elseif self.rel_pos > self.rel then
        self.ended = true
        return 0
    else
        ret = 1 - self.rel_pos/self.rel
        self.rel_pos = self.rel_pos + tick_len
    end
    return ret
end
...
function EnvTri:get_amp(tick_len)
    local ret
    if self.pos < self.att then
        ret = self.pos/self.att
        self.pos = self.pos + tick_len
